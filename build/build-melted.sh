...
# This script builds melted and rugen and many of their dependencies.
# It can accept a configuration file, default: build-melted.conf
...
function set_globals {
  ...
  SUBDIRS="FFmpeg mlt melted rugen"
  ...
  REPOLOCS[2]="git://github.com/trickkiste/melted.git"
  ...
  #####
  # rugen
  CONFIG[6]="./autogen.sh && ./configure --prefix=$FINAL_INSTALL_DIR"
  CFLAGS_[6]=$CFLAGS
  LDFLAGS_[6]=$LDFLAGS

  #####
  # swfdec
  CONFIG[7]="./configure --prefix=$FINAL_INSTALL_DIR --libdir=$FINAL_INSTALL_DIR/lib --disable-gtk --disable-gstreamer"
  CFLAGS_[7]=$CFLAGS
  LDFLAGS_[7]=$LDFLAGS

  #####
  # lame
  CONFIG[8]="./configure --prefix=$FINAL_INSTALL_DIR --libdir=$FINAL_INSTALL_DIR/lib --disable-decoder --disable-frontend"
  CFLAGS_[8]=$CFLAGS
  LDFLAGS_[8]=$LDFLAGS
}
...
function configure_compile_install_subproject {
  ...
  if test "1" = "$NEED_SUDO" ; then
    ...
    TMPNAME=`mktemp -t build-melted.installoutput.XXXXXXXXX`
...
function create_startup_script {
  ...
  if test 0 = $?; then
      ...
      which wget > /dev/null
      if test 0 = $?; then
          cmd $(wget -O - $ARCHIVE | tar -xz) || die "Unable to download startup scripts from $ARCHIVE"
      fi
  fi
